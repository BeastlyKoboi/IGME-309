...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 27:
		...
		break;
	case 'a':
		if (barPos[0] - 10.0f * deltaTime - barSize / 2.0f > 0)
			barPos[0] -= 10.0f * deltaTime;
		break;
	case 'd':
		if (barPos[0] + 10.0f * deltaTime + barSize / 2.0f < canvasSize[0])
			barPos[0] += 10.0f * deltaTime;
		break;
	}
}
...
void update() {
	...
	if (ballIsActive == false) {
		...
		if (barPos[0] - barSize / 2 <= ballPos[0] + radius && 
			barPos[0] + barSize / 2 >= ballPos[0] - radius && 
